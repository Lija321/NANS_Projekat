...
def are_assumptions_satisfied(model, features, labels, p_value_thresh=0.05):
    ...
    if not is_linearity_found:
        print('Linearity assumption is not satisfied.')
    if autocorrelation is not None:
        print('Autocorrelation assumption is not satisfied.')
    if n_dist_type == 'non-normal':
        print('Normality assumption is not satisfied.')
    if e_dist_type == 'non-equal':
        print('Equal variance assumption is not satisfied.')
    if has_perfect_collinearity:
        print('Perfect collinearity assumption is not satisfied.')
